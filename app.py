...
def revolucion_solar():
    ...
    # Initialize jd with user_datetime_utc (this will be the default if year_param is not present)
    ...
    if year_param:
        # ... (your existing logic to find solar_return_iso)
        ...
        # Update jd for all subsequent calculations to the solar return date
    ...
    # --- INICIO DEL CÓDIGO PARA LA FASE LUNAR (MOVER FUERA DEL IF year_param) ---
    luna_pos = swe.calc_ut(jd_for_calculations, swe.MOON)[0][0]
    sol_pos = swe.calc_ut(jd_for_calculations, swe.SUN)[0][0]

    fase_lunar_grados = (luna_pos - sol_pos) % 360

    # Traducción de la fase lunar (your existing logic for 'es' and 'en')
    if lang == 'es':
        if 0 <= fase_lunar_grados < 45:
            fase_lunar = "Luna Nueva"
        elif 45 <= fase_lunar_grados < 90:
